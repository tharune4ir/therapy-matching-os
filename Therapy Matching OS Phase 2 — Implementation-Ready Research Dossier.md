# Therapy Matching OS Phase 2 — Implementation-Ready Research Dossier
...
7. **Dropout baseline = 1-in-5** (Swift & Greenberg 2012). Younger clients, personality/eating-disorder diagnoses, weak alliance early predict dropout. Session-5 retention is the industry-relevant KPI (internal benchmarks target 80%).
...
1. **Embedded ORS/SRS feedback loop** — pre/post-session 4-item scales. Backing: PCOMS (Miller, Duncan); Lambert FIT — clients with feedback are better off than 65% of those without. Complexity: LOW. Highest priority.

2. **Therapist 30–60s intro videos** — warm self-presentation. Backing: warmth predicts client satisfaction (Lab study, Frontiers 2017); choice-effect (Norcross — preference accommodation reduces dropout ~50%). Complexity: LOW. (Industry leaders already utilize therapist profile videos).
...
7. **Therapy readiness assessment + stage-of-change router** — 8-item TTM stage estimator at onboarding; pre-contemplators routed to MI-skilled therapists or to "Therapy Matching OS Listens" first. Backing: Prochaska stage-matched interventions outperform generic. Complexity: LOW.

8. **"Therapy Matching OS Listens" — free 30-min reflective session** — light-touch, non-clinical, with junior psychologist. Backing: Internal testing of this pattern shows positive feedback ("I feel lighter," "perspective"). Reduces hesitation barrier. Complexity: LOW (operational).
...
- **Cold-start fairness:** new therapists should NOT be penalized for missing W24/W25 — use cohort-modality priors and surface "new on Therapy Matching OS" badge instead of low score.